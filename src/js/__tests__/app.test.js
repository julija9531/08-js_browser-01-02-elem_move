...
test.each([
  ...
  ['arh', 'hdfv', Error, 'Введенные данные не являются целыми числами, больше 0!'],
...
])('01. class GameTable/constructor: Ввод некорректных данных(не целых чисел)', (rows, columns, errorType, errorText) => {
  function result() {
...
test('02. class GameTable/constructor: Ввод некорректных данных(без данных)', () => {
  ...
  expect(result).toThrow('Введенные данные не являются целыми числами, больше 0!'); // Проверка текста ошибки
});
...
test('04. class GameTable/mooveImage: Проверка свойств класса:', () => {
  ...
  for (let i = 0; i < 6; i += 1) {
    if (result.elems[i].className === 'table__section__image') {
      imagNum += 1;
      expect(result.imageSection).toBe(i);
    }
  }
